...
class Settings:
    ...
    # Web UI (feature 007): static SPA serving. The built ui/dist is mounted
    # at ui_base_path; non-API routes fall back to index.html (R-05).
    ui_static_dir: Path = field(
        default_factory=lambda: Path(os.environ.get("DF_UI_STATIC_DIR", Path.cwd() / "ui" / "dist"))
    )
    ui_base_path: str = "/"
