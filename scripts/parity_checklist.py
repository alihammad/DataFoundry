...
def processing_parity_check(repo_root: Path) -> dict[str, Any]:
    """Processing-engine cloud-independence check (feature 003, FR-018, SC-007).

    The processing engine (layer model, transformation definitions, promotion
    states, metadata) must be cloud-independent: identical behaviour on both
    clouds. It must never talk to a cloud SDK directly — all layer reads/writes
    go through the ``ProcessingGateway`` abstraction (S3/GCS adapters). This
    check scans the processing engine source for forbidden cloud-SDK imports
    (boto3, google.cloud) and confirms the engine routes reads through the
    gateway.
    """
    processing_dir = (
        repo_root / "control-plane" / "src" / "datafoundry" / "controlplane" / "processing"
    )
    forbidden = ("boto3", "google.cloud", "google.cloud.storage")
    violations: list[str] = []
    for path in sorted(processing_dir.rglob("*.py")):
        text = path.read_text()
        for token in forbidden:
            if token in text:
                violations.append(f"{path.relative_to(repo_root)}: imports {token}")
    ...
    # The engine must route reads through the gateway (not a cloud SDK).
    engine = processing_dir / "engine.py"
    uses_gateway = "gateway" in engine.read_text() if engine.exists() else False

    holds = not violations and uses_gateway
    return {
        "check": "processing_cloud_independence",
        "holds": holds,
        "forbidden_imports": violations,
        "engine_routes_through_gateway": uses_gateway,
    }
...
def render_processing_parity(check: dict[str, Any]) -> str:
    """Render the processing parity check as Markdown."""
    lines = [
        "# Processing Cloud-Independence Check (FR-018, SC-007)",
        "",
        f"Parity holds: **{check['holds']}**",
        "",
        f"Engine routes reads through gateway: **{check['engine_routes_through_gateway']}**",
...
def security_parity_check(repo_root: Path) -> dict[str, Any]:
    """Security-engine cloud-independence check (feature 005, FR-010, SC-007).

    The security engine (classification, protection, keys, tokens, access,
    audit) must be cloud-independent: identical protection outcomes for
    identical data/config on both clouds. It must never talk to a cloud SDK
    directly — all KMS/token-vault/protection operations go through the
    ``SecurityGateway`` abstraction (AWS KMS / GCP Cloud KMS adapters). This
    check scans the security engine source for forbidden cloud-SDK imports
    (boto3, google.cloud) and confirms the engine routes operations through
    the gateway.
    """
    security_dir = repo_root / "control-plane" / "src" / "datafoundry" / "controlplane" / "security"
    forbidden = ("boto3", "google.cloud", "google.cloud.storage")
    violations: list[str] = []
    for path in sorted(security_dir.rglob("*.py")):
        text = path.read_text()
        for token in forbidden:
            if token in text:
                violations.append(f"{path.relative_to(repo_root)}: imports {token}")

    # The engine must route KMS/token operations through the gateway.
    gateway = security_dir / "gateway.py"
    uses_gateway = "class SecurityGateway" in gateway.read_text() if gateway.exists() else False

    holds = not violations and uses_gateway
    return {
        "check": "security_cloud_independence",
        "holds": holds,
        "forbidden_imports": violations,
        "engine_routes_through_gateway": uses_gateway,
    }
...
def render_security_parity(check: dict[str, Any]) -> str:
    """Render the security parity check as Markdown."""
    lines = [
        "# Security Cloud-Independence Check (FR-010, SC-007)",
        "",
        f"Parity holds: **{check['holds']}**",
        "",
        f"Engine routes operations through gateway: **{check['engine_routes_through_gateway']}**",
    ]
    if check["forbidden_imports"]:
        lines.append("")
        lines.append("Forbidden cloud-SDK imports:")
...
def main(argv: list[str] | None = None) -> int:
    ...
    processing_check = processing_parity_check(REPO_ROOT)
    security_check = security_parity_check(REPO_ROOT)
    ...
    if not args.out and not args.json:
        ...
        print()
        print(render_processing_parity(processing_check), end="")
        print()
        print(render_security_parity(security_check), end="")

    return (
        0
        if checklist["parity_holds"]
        and quality_check["holds"]
        and processing_check["holds"]
        and security_check["holds"]
        else 1
    )
